...
const userTypes = [
  {
    ...
    bg: 'bg-gradient-to-br from-brand-orange-500 to-brand-amber-500',
    ...
    <section className="py-20 lg:py-28 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-14">
          ...
          <p className="mt-4 text-brand-navy-500 text-lg max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {userTypes.map(({ icon: Icon, role, headline, description, features, cta, href, bg, textColor }) => (
            <div
              key={role}
              className={`rounded-3xl p-8 flex flex-col ${bg} ${textColor} group`}
            >
              {/* Icon + Role */}
              ...
              {/* Content */}
              <h3 className="text-2xl font-extrabold leading-tight mb-3">{headline}</h3>
              ...
              <ul className="space-y-2 mb-8 flex-1">
                {features.map((f) => (
                  <li key={f} className="flex items-center gap-2.5 text-sm">
                    <ShieldCheck className="w-4 h-4 opacity-60 flex-shrink-0" />
              ...
              <Link
                href={href}
                className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-white/20 hover:bg-white/30 font-semibold text-sm transition-all duration-200 group-hover:gap-3 border border-white/20"
              >
                {cta}
